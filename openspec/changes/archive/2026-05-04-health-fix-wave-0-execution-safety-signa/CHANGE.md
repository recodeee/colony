...
slug: health-fix-wave-0-execution-safety-signa
---

# CHANGE · health-fix-wave-0-execution-safety-signa
...
### Sub-task 0: Prepare storage scope

Update storage files for Health-fix Wave 0: execution_safety + signal_evaporation cleanup.

File scope: packages/storage/src/storage.ts

### Sub-task 1: Update shared infrastructure scope (depends on: 0)

Update remaining non-UI/non-API files for Health-fix Wave 0: execution_safety + signal_evaporation cleanup.

File scope: packages/hooks/src/handlers/pre-tool-use.ts, packages/core/src/scoped-claim.ts, packages/core/src/coordination-sweep.ts, apps/cli/src/commands/health.ts, apps/mcp-server/src/tools/ready-queue.ts
