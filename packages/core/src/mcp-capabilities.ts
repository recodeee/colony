  ...
  for (const source of sources) {
    for (const [name, config] of Object.entries(source.servers)) {
      ...
      if (existing) {
        ...
        existing.capabilities = orderedCapabilities([
          ...existing.capabilities,
          ...classification.capabilities,
        ]);
        existing.sources = orderedCapabilities([...existing.sources, source.id]);
      } else {
        servers.set(name, {
          server: name,
...
export function classifyMcpServer(
  ...
  addIf(haystack, categories, capabilities, /browser|playwright|puppeteer|chrome|ui|screenshot/, [
    'browser/ui',
  ], ['ui-inspection']);
...
function orderedCapabilities(capabilities: string[]): string[] {
