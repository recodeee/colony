...
function findTopErrorReason(rows: ReadonlyArray<McpMetricsAggregateRow>): {
  operation: string;
  count: number;
  ...
  for (const row of rows) {
    for (const reason of row.error_reasons) {
      if (top === null || reason.count > top.count) {
        top = {
    ...
    if (row.error_reasons.length === 0 && row.error_count > 0) {
      if (top === null || row.error_count > top.count) {
        top = {
          operation: row.operation,
          count: row.error_count,
          error_code: null,
